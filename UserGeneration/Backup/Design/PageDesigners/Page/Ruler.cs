...
namespace FastReport.Design.PageDesigners.Page
{
  ...
  internal class HorzRuler : RulerBase
  {
    ...
    protected override void OnMouseUp(MouseEventArgs e)
    {
      ...
      {
        // create new guide
        float f = (e.X - Offset) / ReportWorkspace.Scale;
        if (f < Workspace.Width && e.Y > 5 && e.Y < 20)
        {
          if (ReportWorkspace.SnapToGrid)
            f = (int)(f / ReportWorkspace.Grid.SnapSize) * ReportWorkspace.Grid.SnapSize;
          if (Workspace.Page.Guides == null)
            Workspace.Page.Guides = new FloatCollection();
          Workspace.Page.Guides.Add(f);
  ...
  internal class VertRuler : RulerBase
  {
    ...
    protected override void OnMouseUp(MouseEventArgs e)
    {
      ...
      else
      {
        ...
        if (e.X > 5 && e.X < 20)
        {
          float f = e.Y / ReportWorkspace.Scale - Offset;
          BandBase band = BandAt(f);
          if (band != null)
          {
            f = f - band.Top;
            if (ReportWorkspace.SnapToGrid)
