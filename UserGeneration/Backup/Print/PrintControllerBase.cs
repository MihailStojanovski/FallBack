...
namespace FastReport.Print
{
  internal abstract class PrintControllerBase
  {
    ...
    protected void SetPaperSize(ReportPage page, QueryPageSettingsEventArgs e)
    {
      ...
      SetPaperSize(width, height, e);
    }

    protected void SetPaperSize(float paperWidth, float paperHeight, QueryPageSettingsEventArgs e)
    {
      foreach (PaperSize ps in e.PageSettings.PrinterSettings.PaperSizes)
      {
        ...
        // check if page has the same size
        if (Math.Abs(paperWidth - psWidth) < 5 && Math.Abs(paperHeight - psHeight) < 5)
        {
      ...
      ps1.Width = (int)Math.Round(paperWidth / 25.4f * 100);
      ps1.Height = (int)Math.Round(paperHeight / 25.4f * 100);
      e.PageSettings.PaperSize = ps1;
    }
