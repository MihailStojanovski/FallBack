...
namespace FastReport
{
  ...
  public class GraphicCache : IDisposable
  {
    ...
    {
      int hash = align.GetHashCode() ^ (lineAlign.GetHashCode() << 2) ^ (trimming.GetHashCode() << 3) ^
        (flags.GetHashCode() << 16) ^ (firstTab + 1).GetHashCode() ^ (tabWidth.GetHashCode() << 20);
      ...
      if (result == null)
      {
        ...
        for (int i = 0; i < 64; i++)
          tabStops[i] = tabWidth;
        result.SetTabStops(firstTab, tabStops);
